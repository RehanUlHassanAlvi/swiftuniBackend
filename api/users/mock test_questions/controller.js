...
module.exports.getPendingTestQuestions = async (req, res, next) => {
  try {
    console.log("object")
    const { mock_test_attempted_id } = req.query;
    const values = [mock_test_attempted_id, req.session.userID];
    console.log("values")
    const result = await service.getPendingTestQuestions(values);
    if (result) {
      res.setHeader(
        "Strict-Transport-Security",
        "max-age=31536000; includeSubDomains"
